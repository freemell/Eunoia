            ...
            try {
                ...
                if (data.success) {
                    ...
                    if (data.action === 'connect' && !connected) {
                     ...
                     } else if (data.action === 'limit_order' && data.params && publicKey) {
                         try {
                             const orderResponse = await fetch('/api/limit-orders/create', {
                                 method: 'POST',
                                 headers: { 'Content-Type': 'application/json' },
                                 body: JSON.stringify({
                                     ...
                                     tokenSymbol: data.params.tokenSymbol,
                                     orderType: data.params.orderType,
                                     triggerType: data.params.triggerType,
                                     triggerValue: data.params.triggerValue,
                                     amount: data.params.amount || 'all',
                                     amountType: data.params.amountType || 'fixed',
                                 }),
                             });
                             ...
                             if (orderData.success) {
                                 setMessages(prev => [...prev, {
                                     role: 'assistant',
                                     content: `✅ Limit order created successfully! ${orderData.message}`
                                 }]);
                             } else {
                                 setMessages(prev => [...prev, {
                                     role: 'assistant',
                                     content: `❌ Failed to create limit order: ${orderData.error}`
                                 }]);
                             }
                         } catch (error) {
...
`).join('')}
                    ...
                    const errorMessage = data.error || data.response || 'Unknown error occurred';
                    setMessages(prev => [...prev, {
                        role: 'assistant',
                        content: data.response || `Sorry, I received an error from the AI: ${errorMessage}. Please try again.`
                    }]);
                }
            } catch (error) {
